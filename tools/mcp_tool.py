...
class MCPServerTask:
    ...
    async def run(self, config: dict):
        ...
        initial_retries = 0
        backoff = 1.0
        persistent_reconnect = _parse_boolish(
            config.get("persistent_reconnect", False),
            default=False,
        )
        ...
        while True:
            try:
                ...
                if retries > _MAX_RECONNECT_RETRIES:
                    ...
                    logger.warning(
                        "MCP server '%s' failed after %d reconnection attempts, "
                        "but persistent_reconnect is enabled; retrying in %.0fs: %s",
                        self.name, _MAX_RECONNECT_RETRIES, backoff, exc,
                    )
                    retries = _MAX_RECONNECT_RETRIES
                else:
                    logger.warning(
