    ...
    def _summarize_api_error(error: Exception) -> str:
        ...
        if response is not None:
            try:
                ...
                # httpx raises ResponseNotRead when this is a non-OK streaming
                # response whose body was already consumed by the provider
                # adapter's bounded error reader.  Error summarization must
                # never mask the original, actionable provider exception.
                snippet = ""
            if snippet:
                status_code = getattr(error, "status_code", None)
                prefix = f"HTTP {status_code}: " if status_code else ""
